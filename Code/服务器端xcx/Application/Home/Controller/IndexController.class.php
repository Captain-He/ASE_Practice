...
class IndexController extends Controller {
    ...
    public function getnewsid(){
    	$category = M('article')->select();
    	echo(json_encode($category,true));
    }

    //获取新闻
    public function getnews(){
         //$category = M('qw_category')->field('id,pid,name')->order('o asc')->select();
    	//$category = M('qw_article')->select();
    	//echo(json_encode($category,true));

    	$id = 'aid='.I('id');
    	 $category = M('article')->where($id)->select();
    	 $title = $category['0']['title'];
    	 $zhaiyao = $category['0']['description'];
    	 $pic = 'https://www.caption-he.com.cn/xcx/'.$category['0']['thumbnail'];
    	 $neirong = $category['0']['content'];
    	 $time = date("Y-m-d H:i:s",$category['0']['t']);
    	$data = '<html>
 <head>
  <meta charset=UTF-8>
 </head>
 <body>
 <div id="" class="" style="width:330px; margin:2px auto;">
 <h1 style="font-family:"楷体";">'.$title.' | 云笔记快讯</h1>
 <p style="color:#ccc;"><abbr title="司马迁迁" style="text-decoration:none;
      color:#52f7f2;">司马迁迁 .&nbsp;&nbsp;</abbr>'.$time.'</p>
 <p style="background:#f2f2f2;text-align:left;width:300px;height:70px;"><b>摘要:</b> '.$zhaiyao.' </p>
 <img src="'.$pic.'" width="100" height="" border="0" alt="" style="margin:40px 0;">
 <p class="box" style="line-height:70px;
   font-size:14px;">'.$neirong.'</p>
 </div>
 <div style="width:335px; margin:10px auto;"><p style="background:#f2f2f2;text-align:bottom;width:300px;height:30px;color:gray;">文章来自云笔记|copyright@caption-he </p></div>
 </body>
</html>
';
echo $data;
	...
	public function search(){
		...
		$contentjson = array();
		$mysql = M('Dirary');
		$result = $mysql -> query("select *from dirary where openid ='$open'");
